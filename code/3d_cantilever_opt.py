...
thickness = 100.0 # [mm]
width = 25.0/2.0 # [mm]
resolution = .2 # [Nodes/mm] = 1 [Nodes/cm]
load = Constant((0.0,-500.0/2.0, 0.0)) # [N]

folder_name = './3d_cantilever_opt_results'
...
mesh = BoxMesh(Point(0.0,0.0,0.0), Point(length, thickness, width), resX, resY, resZ)
...
loadRegion = rs.GetPlanarBoundary.from_points(\
        Point(length-40.0, thickness, 0.0),\
        Point(length-40.0, thickness, width),\
        Point(length, thickness, width),\
...
frozenLoadRegion = rs.GetCuboidRegion(\
        Point(length, thickness, width),\
        Point(length - 40.0, thickness, width),\
        Point(length, thickness - 10.0, width),\
        Point(length, thickness, 0.0))
